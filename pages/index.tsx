...
export default function Home() {
  ...
  const fetchChuck = async () => {
    console.log('Fetching chucks...');
    const chuckData = await getChuck();
    console.log('Fetched Chuck:', chuckData);
    setChuck(chuckData);
  };
  ...
  return (
    <div className={styles.container}>
      <main className={styles.main}>
        <img src={chuckGif} alt="Chuck Norris" className={styles.gif} />
        <h1 className={styles.title}>Frase Aleatoria de Chuck Norris</h1>
        <div className={styles.grid}>
          {chuck.map((item, index) => (
            <div key={item.id} className={styles.card}>
              <h2>{item.value}</h2>
            </div>
          ))}
        </div>
        <button className={styles.button} onClick={handleNewJoke}>Nueva Frase</button>
      </main>
    </div>
  );
}
